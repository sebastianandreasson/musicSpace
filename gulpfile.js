...
function compileLess() {
  const less = require('gulp-less')(paths.styles)
...
function notifyLivereload(event) {
  const fileName = require('path').relative(devServer.root, event.path)
  lr.changed({ body: { files: [fileName] } })
  const serverName = devServer.server === '0.0.0.0' ? '127.0.0.1' : devServer.server
  gutil.log('Notified live reload for http://' + serverName + ':' + devServer.port)
}

function getRandomPortBasedOnPath(seed) {
  const sillyHash = getHash(__dirname)
